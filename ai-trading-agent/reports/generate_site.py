...
def render(report: dict) -> str:
    ...
    signals = "".join(f"<article class='card'><div class='row'><h3>{escape(str(item['symbol']))}</h3><span class='badge'>{float(item['score']):.1f}/100</span></div><p>{escape(str(item['direction']))} · {escape(str(item.get('sector', 'Unknown')))}</p><p class='sector-trend'>Sector: {escape(candidate_sector(item))} {candidate_trend(item)}</p><ul>{''.join(f'<li>{escape(str(reason))}</li>' for reason in item.get('reasons', []))}</ul>{candidate_llm(item)}</article>" for item in report.get("signals", []))
