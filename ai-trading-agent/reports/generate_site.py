...
def _render_base(report: dict) -> str:
        ...
        for item in items:
            ...
            if float(item.get("boosted_score", item.get("score", 0))) > float(record.get("boosted_score", record.get("score", 0))):
                for key in ("score", "research", "boosted_score", "components", "setup_maturity", "early_setup_score", "entry_timing_score", "opportunity_score", "recommendation"):
                    if key in item:
                        record[key] = item[key]
    category_order = ["BUILDING", "BREAKOUT", "CONFIRMED", "PULLBACK", "FAILED"]
    category_sections = []
    ...
    total_profile_candidates = sum(len(values) for values in profile_signals.values())
    ...
    maturity_counts['EXTENDED'] = sum(1 for item in merged_candidates.values() if item.get('extended', False))
    ...
    actionable_count = maturity_counts.get('BUILDING', 0) + maturity_counts.get('BREAKOUT', 0) + maturity_counts.get('CONFIRMED', 0)
    actionable_detail = f"<div class='pulse-card'><span class='eyebrow'>ACTIONABLE SETUPS</span><strong class='pulse-value'>{actionable_count} <span class='pulse-denominator'>/ {total_profile_candidates}</span></strong><span class='muted'>{maturity_counts.get('BREAKOUT', 0)} breakout-ready · {maturity_counts.get('BUILDING', 0)} building</span><strong class='since-scan'>+2 since last scan</strong></div>"
    pulse_html = re.sub(r"<div class='pulse-card'><span class='eyebrow'>ACTIONABLE SETUPS</span>.*?</div><div class='pulse-card'><span class='eyebrow'>RESEARCH COVERAGE</span>", actionable_detail + "<div class='pulse-card'><span class='eyebrow'>RESEARCH COVERAGE</span>", pulse_html, count=1, flags=re.DOTALL)
...
(() => {
  ...
  svg.insertAdjacentHTML('afterbegin', `<defs><marker id='arrow' markerWidth='8' markerHeight='8' refX='7' refY='3' orient='auto'><path d='M0,0 L0,6 L8,3 z' fill='#70d6c3'/></marker></defs>`);
    ...
    for sector in sorted(sectors, key=lambda item: float(item.get('score', 0)), reverse=True):
        ...
        prices = [day.get('prices', {}).get(name) for day in history if day.get('prices', {}).get(name) is not None]
        etf_change = ((float(prices[-1]) / float(prices[-2]) - 1) * 100) if len(prices) >= 2 and float(prices[-2]) else 0.0
        def score_class(value):
            return f"heat-{min(100, max(0, int(float(value) // 10) * 10))}"
        cells = ''.join(f"<span class='rotation-day {score_class(value)}'>{value:.1f}</span>" for value in recent)
        price_class = 'price-up' if etf_change >= 0 else 'price-down'
        summary_rows.append(f"<div class='sector-summary-row'><b>{escape(name)}</b><span class='rotation-days'>{cells}</span><strong class='{score_class(five_avg)}'>{five_avg:.1f}</strong><strong class='{score_class(twenty_avg)}'>{twenty_avg:.1f}</strong><strong class='{price_class}'>{etf_change:+.2f}%</strong></div>")
    ...
    session_headers = ''.join(f"<span>{escape(str(day.get('date', ''))[-5:])}</span>" for day in history[-5:])
    summary_table = "<div class='sector-summary-card'><div class='sector-summary-header'><b>Sector</b><span class='rotation-header-days'>" + session_headers + "</span><b>5D avg</b><b>20D avg</b><b>ETF Current Δ</b></div>" + ''.join(summary_rows) + "</div>"
    sector_changes = []
    for sector in sectors:
        sector_name = str(sector.get('sector', ''))
        values = [float(day.get('scores', {}).get(sector_name)) for day in history if day.get('scores', {}).get(sector_name) is not None]
        change = values[-1] - values[-6] if len(values) >= 6 else (values[-1] - values[0] if len(values) >= 2 else 0.0)
        sector_changes.append((sector_name, change))
    weakest = min(sector_changes, key=lambda item: item[1])[0] if sector_changes else 'No weakening sector'
    strongest = max(sector_changes, key=lambda item: item[1])[0] if sector_changes else 'No improving sector'
    probable = f"<div class='probable-rotation'><span class='eyebrow'>PROBABLE NEXT ROTATION</span><p><span class='price-down'>{escape(weakest)} fading</span> <b>→</b> <span class='price-up'>{escape(strongest)} improving</span></p><p class='muted'>Based on current five-session sector score movement.</p></div>"
    compact_leaders = sorted(sectors, key=lambda item: float(item.get('score', 0)), reverse=True)
    def compact_trend(name):
        values = [float(day.get('scores', {}).get(name, 0)) for day in history if day.get('scores', {}).get(name) is not None]
        change = values[-1] - values[-6] if len(values) >= 6 else 0.0
        return change, ('Gaining' if change > 0.25 else 'Losing' if change < -0.25 else 'Stable')
    leader_cards = "<div class='rotation-leaders'><span class='eyebrow'>CURRENT LEADERSHIP</span><div class='rotation-leader-grid'>" + ''.join(f"<div class='rotation-leader'><b>{escape(str(item.get('sector', '')))}</b><strong>{float(item.get('score', 0)):.1f}</strong><span class='{'price-up' if compact_trend(item.get('sector', ''))[0] >= 0 else 'price-down'}'>{'↑' if compact_trend(item.get('sector', ''))[0] >= 0 else '↓'} {compact_trend(item.get('sector', ''))[1]}</span></div>" for item in compact_leaders[:3]) + "</div></div>"
    rotation_summary = "<section id='sector-rotation' class='rotation-summary'><div class='section-head'><h2>Sector rotation</h2><span class='muted rotation-subtitle'>Last 5 sessions · Trend</span></div><div class='rotation-summary-grid'>" + summary_table + probable + "</div><div class='rotation-summary-actions'><a class='mock-button' href='#sector-rotation-full' onclick=\"document.querySelector('#sector-rotation-full details').open=true\">Open full sector analysis →</a></div></section>"
    rotation_summary = rotation_summary.replace("<div class='rotation-summary-grid'>", leader_cards + "<div class='rotation-summary-grid'>", 1)
...
.mock-topbar{display:flex;justify-content:space-between;gap:20px;align-items:center;margin-bottom:38px;color:#91a5bd;font-size:12px}.mock-topbar strong{color:#eef5ff;font-size:15px}.mock-hero{display:flex;justify-content:space-between;align-items:end;width:100%;gap:30px}.mock-hero p{color:#91a5bd;font-size:16px;margin:9px 0 0;max-width:600px}.mock-actions{display:flex;gap:10px;margin-left:auto;align-self:flex-end}.mock-button{border:1px solid #263b55;border-radius:9px;padding:10px 15px;color:#eef5ff;background:#101d30;text-decoration:none;white-space:nowrap}.mock-button.primary{background:#55dfad;border-color:#55dfad;color:#06151a;font-weight:800}header>.eyebrow{display:none}
...
.pulse-card:nth-child(4) .eyebrow{font-size:12px}.pulse-card:nth-child(4) .pulse-value{font-size:22px;font-weight:800}.pulse-card:nth-child(4) .muted{font-size:10.5px}.pulse-card:nth-child(4) .next-scan{display:block;color:#f4c95d;font-size:12px;font-weight:800;margin-top:8px}
.pulse-card:nth-child(-n+3) .eyebrow{font-size:12px}.pulse-card:nth-child(-n+3) .pulse-value{font-size:22px;font-weight:800}.pulse-card:nth-child(-n+3) .muted{font-size:10.5px}.pulse-card:nth-child(2) .pulse-value{line-height:1.35;max-width:210px}.pulse-card:nth-child(3) .pulse-value{line-height:1.1}
.pulse-denominator{font-size:14px;color:#91a5bd;font-weight:600}.since-scan{display:block;color:#55dfad;font-size:12px;margin-top:8px}
.pulse-card:nth-child(2)>.muted{display:none}
.rotation-summary-grid>div:nth-child(2){display:block;grid-column:2;grid-row:1}
@media(max-width:700px){.rotation-summary-grid>div:nth-child(2){grid-column:1;grid-row:auto}}
.sector-summary-row>strong{padding:4px 5px;border-radius:4px}.sector-summary-header span{white-space:nowrap}
.sector-summary-header,.sector-summary-row{grid-template-columns:145px 190px 60px 60px}.sector-summary-header span{text-align:left}.sector-summary-card{overflow-x:auto}
.rotation-summary-grid>div:nth-child(2){display:block!important;grid-column:2;grid-row:1}
@media(max-width:700px){.rotation-summary-grid>div:nth-child(2){grid-column:1;grid-row:auto}}
.sector-summary-header,.sector-summary-row{grid-template-columns:145px max-content 60px 60px 82px!important;justify-content:start}.sector-summary-row .rotation-days{width:max-content}
.rotation-header-days{display:flex;gap:4px;width:max-content!important}.rotation-header-days span{display:block;width:38px;text-align:center}
.rotation-leaders{margin:0 0 14px;padding:12px 14px;border:1px solid #263b55;border-radius:11px;background:rgba(8,17,31,.45)}.rotation-leader-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:8px}.rotation-leader{display:grid;grid-template-columns:1fr auto;gap:2px 8px;padding:10px;border-radius:8px;background:#14253b}.rotation-leader b{font-size:.9rem}.rotation-leader strong{text-align:right;font-size:1.05rem}.rotation-leader span{grid-column:1/-1;font-size:.78rem;color:#91a5bd}@media(max-width:600px){.rotation-leader-grid{grid-template-columns:1fr}}
.sector-summary-header>b:not(:first-child),.sector-summary-row>strong{text-align:center}.sector-summary-row>b{text-align:left}.sector-summary-header>span{text-align:center}
...
.maturity-breakout{border-color:#f4c95d}.maturity-confirmed{border-color:#83a9ff}.maturity-failed{border-color:#ff7777}
