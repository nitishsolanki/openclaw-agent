...
(() => {
  ...
  svg.insertAdjacentHTML('afterbegin', `<defs><marker id='arrow' markerWidth='8' markerHeight='8' refX='7' refY='3' orient='auto'><path d='M0,0 L0,6 L8,3 z' fill='#70d6c3'/></marker></defs>`);
    ...
    leader_cards = "<div class='rotation-leaders'><span class='eyebrow'>CURRENT LEADERSHIP</span><div class='rotation-leader-grid'>" + ''.join(f"<div class='rotation-leader'><b>{escape(str(item.get('sector', '')))}</b><strong>{float(item.get('score', 0)):.1f}</strong><span class='{'price-up' if trends.get(item.get('sector'), ('', 0, 0))[1] >= 0 else 'price-down'}'>{'↑' if trends.get(item.get('sector'), ('', 0, 0))[1] >= 0 else '↓'} {trends.get(item.get('sector'), ('Stable', 0, 0))[0]}</span></div>" for item in ranked_sectors[:3]) + "</div></div>"
