...
def _render_base(report: dict) -> str:
    ...
    candidate_sections = "<section id='full-candidates'><div class='section-head'><h2>Opportunity command center</h2><span class='muted'>Top setups by maturity</span></div><div class='maturity-grid'>" + "".join(category_sections) + "</div></section>"
    full_rows = []
    for category in category_order:
        items = [item for item in merged_candidates.values() if str(item.get('setup_maturity', 'BUILDING')) == category]
        items.sort(key=lambda item: float(item.get('boosted_score', item.get('score', 0))), reverse=True)
    ...
    full_list_html = "<section id='candidate-details'><div class='section-head'><h2>Full candidate lists</h2><a href='#full-candidates'>Back to top candidates ↑</a></div>" + ''.join(full_rows) + "</section>"
    candidate_sections = candidate_sections.replace("href='#full-candidates'", "href='#candidate-details'") + full_list_html
...
(() => {
  ...
  svg.insertAdjacentHTML('afterbegin', `<defs><marker id='arrow' markerWidth='8' markerHeight='8' refX='7' refY='3' orient='auto'><path d='M0,0 L0,6 L8,3 z' fill='#70d6c3'/></marker></defs>`);
    ...
    return html.replace("</main>", replacement + "</main>", 1)
