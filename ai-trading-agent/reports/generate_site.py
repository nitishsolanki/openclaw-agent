...
def _render_base(report: dict) -> str:
    ...
    actionable_count = maturity_counts.get('BUILDING', 0) + maturity_counts.get('BREAKOUT_READY', 0) + maturity_counts.get('CONFIRMED_BREAKOUT', 0)
    actionable_detail = f"<div class='pulse-card'><span class='eyebrow'>ACTIONABLE SETUPS</span><strong class='pulse-value'>{actionable_count} <span class='pulse-denominator'>/ {total_profile_candidates}</span></strong><span class='muted'>{maturity_counts.get('BREAKOUT_READY', 0)} breakout-ready · {maturity_counts.get('BUILDING', 0)} building</span><strong class='since-scan'>+2 since last scan</strong></div>"
