...
def render(report: dict) -> str:
    ...
    def candidate_llm(item):
        analysis = item.get("research", {})
        if not analysis or analysis.get("status") == "unavailable":
            return ""
        catalysts = ", ".join(str(value) for value in analysis.get("catalysts", [])[:3])
        risks = ", ".join(str(value) for value in analysis.get("risks", [])[:3])
        conviction = escape(str(analysis.get("conviction", "Not provided")))
        summary = escape(str(analysis.get("summary", analysis.get("thesis", ""))))
        return f"<div class='ai-note'><strong>Research: {conviction} · {float(analysis.get('research_score', 0)):.1f}/100</strong><p>{summary}</p><small><b>Catalysts:</b> {escape(catalysts or 'None listed')}</small><br><small><b>Risks:</b> {escape(risks or 'None listed')}</small></div>"
